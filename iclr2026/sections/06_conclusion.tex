...
added to the loss. Switching the two guards on and off independently is
decisive in one direction only: removing \emph{both} collapses success to
exactly $0.000$ on both tasks, while either alone recovers essentially all of
the gain --- indeed removing the anchor and keeping only the step bound is
among our strongest settings ($0.803$ on t65). We therefore claim the
necessity of an action-space constraint, and not the superiority of the
particular one we ship.
