...
diffusion base, where dropping it into DICE-RL raises success from $0.861$ to
$0.937$. Its \emph{size} of benefit, however, is not base-agnostic: on the
...
more than the location of the constraint. A hard projection onto
$\|r\|\le\rho$ does as well as our dead-zone anchor and better on one task
(\cref{tab:locus}), so the anchor is one workable instrument rather than the
necessary one. The distinction is not cosmetic. A step limit constrains
