...
\Cref{sec:toy} left us with a concrete requirement: use the critic's
gradient as a \emph{direction}, but never let its \emph{magnitude} decide
how far the policy moves, and bound that movement where the damage
happens --- in action space. \method{} meets it by borrowing the
operation that pretrained the policy in the first place: \emph{move
action samples along a velocity field, then regress the network onto the
moved samples.} Fine-tuning becomes one more field added to that
transport.

Two fields do all the work, and they are the paper's main contribution.
The \textbf{reward field} $V_Q$ says which way each action should move to
score better, and is clipped so no single update can be large. The
\textbf{anchor field} $V_{\mathrm{anc}}$ pulls the policy back toward its
base, but only once it has strayed beyond a set radius --- inside that
radius it is silent, so it never fights the reward field. Neither is a
