...
The contribution is the \textbf{anchor field} $V_{\mathrm{anc}}$. It pulls
the policy back toward its frozen base, but only once the residual has
strayed beyond a radius $\rho$; inside that radius it is exactly zero, so it
never competes with the reward signal in the region where the critic has
data. \cref{sec:exp-ablation} isolates it: with the anchor and without the
step bound the method is unchanged, and with the step bound and without the
anchor it fails completely. The \textbf{reward field} $V_Q$ says which way
each action should move to score better, and is norm-clipped for numerical
safety --- we retain the clip, but report that it is not what makes the
update work. Neither field is a
loss term: both are displacements applied to actions before the
regression, which is exactly why the anchor cannot be out-shouted the way
a behavior-cloning penalty was in \cref{sec:toy}. No likelihood is
required, and no critic gradient is ever backpropagated into the actor.
...
\paragraph{The step bound, and why it is not the mechanism.}
It would be natural to assume the clip in \cref{eq:update} is what makes
this safe. Under $\mathcal{L} = \mathbb{E}[-Q]$ the actor's step scales
with $\|\nabla_a Q\|$, whereas here the policy provably moves at most
$\delta_{\mathrm{tot}}$ per particle per update, a guarantee
parameter-space gradient clipping cannot give. The guarantee is real. It is
also, on its own, useless: a drift assembled from many individually
permitted steps is still a drift, because the critic keeps pointing the same
way and nothing returns the policy. Switching the clip off while keeping the
anchor leaves the method unchanged; switching the anchor off while keeping
the clip destroys it (\cref{sec:exp-ablation}). We therefore keep the clip
for numerical safety and attribute the behaviour to the anchor.
