...
The vehicle for that bound is an existing primitive, which we adopt rather
than claim: \emph{move action samples along a velocity field, then regress
the network onto the moved samples}. This is how the action-gradient family
improves a policy \citep{dipo2024,psenka2024qsm}. What that family does not
do, and what this section contributes, is constrain the displacement: in
DIPO the improved action is wherever the gradient step lands, and the policy
is then regressed onto it.

Nothing below is specific to a policy class. The update needs a frozen map
from noise to actions, the ability to draw $K$ samples from it, and a
critic; it never needs a likelihood, a denoising objective, or access to the
generator's internals. It therefore applies to a one-step map, a few-step
flow-matching sampler, or a many-step diffusion policy without modification,
and we verify two of these empirically (\cref{sec:exp-generality}). One
convenience is worth noting without leaning on it: a drifting model is
itself \emph{pretrained} by field-target regression
\citep{deng2026drifting}, so on that particular base the fine-tuner reuses
machinery the policy already has. That is an implementation economy, not a
requirement, and none of our claims depend on it.
...
In words, one update does four things. \textbf{(1) Propose:} draw $K$
noise samples and push them through the frozen base plus the current
residual to get a cloud of candidate actions for this state.
\textbf{(2) Score and displace:} ask the critic which direction improves
each candidate, clip that direction to length $\delta_Q$, and add the
anchor's pull if the residual has strayed past its radius; clip the
resulting total displacement to $\delta_{\mathrm{tot}}$.
\textbf{(3) Freeze the targets:} the displaced actions are detached ---
they are data now, not a computation graph. \textbf{(4) Regress:} train
the residual head to output those displaced targets. The critic is
trained separately (\cref{sec:critic}); it is never differentiated
through the actor, only queried.
...
\emph{what kind} of function the network is asked to represent.
