...
\begin{figure}[t]
  ...
  \caption{\textbf{\method{} in three steps} (schematic). \emph{Left:} the
  frozen base maps noise to $K$ candidate actions, which lie on the thin band
  of actions the demonstrations support. \emph{Middle:} the critic's
  gradient points \emph{off} that band (faint arrows, unbounded); \method{}
  keeps only its direction and takes a step of length at most $\delta_Q$
  (solid), then the dead-zone anchor pulls back any candidate that has left
  the leash of radius $\rho$ (green). \emph{Right:} the displaced targets are
  detached and the residual head is regressed onto them, so the policy moves
  a bounded distance and stops. The base is any frozen noise-to-action map.}
  \label{fig:method}
\end{figure}
