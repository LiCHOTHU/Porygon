...
The contribution is the \textbf{anchor field} $V_{\mathrm{anc}}$: a pull back
toward the frozen base that is \emph{silent} while the residual stays within a
radius $\rho$ and engages only outside it (\cref{sec:anchor} gives its form).
It is not an ad hoc construction --- it is the negative gradient of a hinged
radial potential, and at $\rho = 0$ it reduces to the ordinary quadratic
residual regularizer the backpropagated control already uses. The dead zone is
thus a \emph{hinge generalization} of standard $L_2$ residual regularization,
and the experiments say that where $\rho$ sits on that interpolation decides
whether fine-tuning works at all (\cref{sec:exp-ablation}).

The \textbf{reward field} $V_Q$ says which direction improves each action. It
is norm-clipped, and we keep the clip, but we do not attribute the method's
behaviour to it: switching the clip off while retaining the anchor leaves
performance unchanged, whereas switching the anchor off while retaining the
clip removes the gains entirely (\cref{sec:exp-ablation}).
...
The distinction matters less than it might, because the clip turns out not
to be the operative component in any case. A drift assembled from many
individually permitted steps is still a drift, since the critic keeps
pointing the same way and nothing returns the policy. Switching the clip off while keeping the
