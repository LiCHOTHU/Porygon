...
``---'' = not measured. $^{\P\P}$\textbf{the diffusion base has been retrained} and now measures $0.600$ on $300$ episodes ($24000$ epochs, $[1024,1024,1024]$ trunk with a conditioning MLP, $20$ denoising steps), independently confirmed at $0.550$--$0.600$ by the step-$0$ evaluation of all six fine-tuning seeds. Every diffusion row is being re-run from this checkpoint ($3$ seeds for the paired rows); the previous-base rows remain until those land and will then be replaced.}
\label{tab:baselines}
\footnotesize
...
diffusion $+$ DICE-RL (prev.\ base)$^{\S}$   & diffusion & 0.279 & 0.861 & $+0.582$ \\
diffusion $+$ DICE-RL $+$ \method{} (prev.\ base)$^{\S}$ & diffusion & 0.279 & \textbf{0.937} & $\mathbf{+0.658}$ \\
diffusion $+$ DICE-RL (retrained base)$^{\P\P}$   & diffusion & 0.600 & training & --- \\
diffusion $+$ DICE-RL $+$ \method{} (retrained base)$^{\P\P}$ & diffusion & 0.600 & training & --- \\
\addlinespace
...
baseline. $^{\ddagger}$\textbf{transport is a tie and is still training.} Unlike the can and square blocks, which are converged, these are a snapshot at a matched $256$K steps --- the largest step at which all six runs have an evaluation --- and both arms are still climbing (one \method{} seed reaches $0.46$ by $480$K). The three-seed means are $0.323$ (DICE-RL) against $0.331$ (\method{}), and the seed ranges overlap completely ($0.313$--$0.334$ against $0.312$--$0.363$), so on this task we claim no advantage. We report it because a third task that does \emph{not} separate the two update rules is as much part of the picture as the two that do.}
\label{tab:matched}
\footnotesize
...
\toprule
 & & \multicolumn{3}{c}{can} & \multicolumn{3}{c}{square} & \multicolumn{3}{c}{transport$^{\ddagger}$} \\
\cmidrule(lr){3-5}\cmidrule(lr){6-8}\cmidrule(lr){9-11}
actor update & how the critic is used & s42 & s43 & s44 & s42 & s43 & s44 & s42 & s43 & s44 \\
\midrule
frozen base, no RL & critic never queried & \multicolumn{3}{c}{0.877} & \multicolumn{3}{c}{0.382} & \multicolumn{3}{c}{---} \\
DICE-RL actor \emph{(baseline)} & $Q$ differentiated \emph{through} the policy & 0.970$^{\dagger}$ & 0.946$^{\dagger}$ & 0.940 & 0.870 & 0.909 & 0.861 & 0.313 & 0.321 & 0.334 \\
\textbf{\method{} actor \emph{(ours)}} & $Q$ only \emph{queried}; targets displaced, bounded, regressed & \textbf{0.988} & \textbf{0.993} & \textbf{0.978} & \textbf{0.912} & \textbf{0.930} & \textbf{0.929} & 0.312 & 0.317 & 0.363 \\
\bottomrule
\end{tabular}}
\end{table}
...
(\cref{tab:baselines}).
\textbf{The anchor is what the update cannot do without; the step clip is a
task-dependent refinement.} Across all five tasks the two rows lacking the
anchor sit at exactly $0.000$: with no restoring term the transported targets
leave the base's support and the regressed residual never recovers, and the
step clip alone does not prevent it. That is the claim this factorial
establishes, and five tasks establish it far more firmly than the two we
previously reported.
The clip is a different matter. Adding it to the anchor helps on three tasks
(ketchup $+0.240$, book-into-caddy $+0.207$, bowl $+0.033$) and hurts on two
(red mug $-0.210$, juice $-0.040$), for a mean of $0.703$ against $0.657$. So
full \method{} is the better default --- it wins on three of five and on
average --- but ``both guards are necessary'' overstates it: only the anchor is
necessary, and we claim no more than that. Three seeds per cell.}
...
reference and should not be compared across columns.
$^{\dagger}$single seed ($42$), $300$ episodes, and --- unlike every other
number in this paper --- \emph{not} read at a matched step: repeated
preemption and resume left these three arms with disjoint evaluation windows
(hinge $464$--$640$K, projection $656$--$880$K, always-on pull $672$--$896$K),
so no common step exists. We instead quote each arm at its own converged
plateau (mean of its last five evaluations); all three are flat to within
$\pm 0.013$ over their final $200$K steps, which is what makes the comparison
meaningful despite the offset. The hinge's $0.991$ is far above anything else
we measure on square and rests on one seed; we flag it as provisional.
``---'' = not measured.}
...
\quad always-on pull ($\rho{=}0$)           & action target   & 0.547$^{\dagger}$ & 0.549 & 0.601 \\
\quad hinge $(\|r\|-\rho)_{+}^{2}$          & loss            & \textbf{0.991}$^{\dagger}$ & \textbf{0.837} & 0.700 \\
\quad hard projection $\Pi_{\|r\|\le\rho}$  & action target   & 0.573$^{\dagger}$ & 0.790 & \textbf{0.733} \\
