...
\begin{table}[t]
\centering
\caption{\textbf{Recipe sensitivity on LIBERO: every component earns its
place.} Each row changes exactly one component of the frozen recipe;
$\Delta$ is against the full recipe, so a negative number in the top block
means the component was helping. Powered evaluation, last-3 checkpoints.
Read the top block first: removing any single component costs up to $23$
points, and which component matters most is task-dependent (the dead zone on
the red-mug task, multi-step returns on ketchup). The bottom block shows the
recipe is not at a local optimum --- both loosenings help --- but we ship
the shared recipe rather than tuning per task (\cref{app:eta-sweep}).}
