...
$^{\dagger\dagger}$both paired rows are still improving and quoted at their
current budgets (mean of last three evaluations, three seeds each): DICE-RL at
$368$--$512$K ($0.946/0.959/0.956$), \method{} at $128$K
($0.781/0.871/0.811$) --- its runs iterate ${\sim}3.5\times$ slower, so it
trails in wall-clock, not in matched compute. At the matched $128$K point
\method{} reads $0.821$ against DICE-RL's $0.796$. The pair will be re-quoted
at a single matched budget when \method{} reaches DICE-RL's step count.
$^{*}$the classic baselines' harness scores success more strictly (a full
action chunk inside the success state) and reads this same checkpoint at
$0.348$ under that rule; their fine-tuned numbers are on that stricter scale
and their $\Delta$ is computed against $0.348$, not $0.600$. DPPO's and the
...
\midrule
DIPO \citep{dipo2024}     & diffusion & 0.600$^{*}$ & 0.208$^{\circ}$ & $-0.140$ \\
QSM \citep{psenka2024qsm} & diffusion & 0.600$^{*}$ & 0.000 & $-0.348$ \\
DQL                       & diffusion & 0.600$^{*}$ & 0.116$^{\circ}$ & $-0.232$ \\
IDQL                      & diffusion & 0.600$^{*}$ & 0.307$^{\circ}$ & $-0.050$ \\
AWR                       & generic   & 0.600$^{*}$ & 0.338$^{\circ}$ & $+0.042$ \\
DPPO \citep{ren2024dppo} & diffusion & 0.600 & 0.672$^{\circ}$ & $+0.072$ \\
\midrule
\multicolumn{5}{l}{\emph{adding \method{} to DICE-RL, paired within each base}} \\
diffusion $+$ DICE-RL \citep{dicerl2026}$^{\P\P}$   & diffusion & 0.600 & \textbf{0.95}$^{\dagger\dagger}$ & $\mathbf{+0.35}$ \\
diffusion $+$ DICE-RL $+$ \method{}$^{\P\P}$        & diffusion & 0.600 & 0.82$^{\dagger\dagger}$ & $+0.22$ \\
\addlinespace
