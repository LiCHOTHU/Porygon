...
overlap}: full \method{}'s three evaluation seeds are $0.62$, $0.68$, $0.58$
and anchor-only's are $0.40$, $0.41$, $0.35$, so \method{}'s \emph{worst}
seed beats the ablation's \emph{best} by $0.17$, with the two step-clip rows
at exactly $0.000$ beneath both. That single task is the cleanest evidence
that the pair is required, and it does not rely on averaging.
``---'' = not measured.}
