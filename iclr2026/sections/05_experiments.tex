...
$^{\mathrm{z}}$this row runs \method{}'s zeroth-order scoring variant (\cref{sec:reward-field}) rather than the headline gradient variant. ``---'' = not measured.}
\label{tab:baselines}
\footnotesize
...
the update rule's advantage is largest where the critic is weakest, and
transport is where it is strongest. \Cref{app:regime} gives the full
account of when the bound helps and when it must cost.}
\label{tab:matched}
\footnotesize
...
$^{\dagger}$single seed ($42$), $300$ episodes, quoted at its converged
plateau. The hinge's value is far above anything else we measure on square
and rests on one seed; we flag it as provisional. An audit found the square
runs of the always-on-pull and hard-projection variants used a different
reward-field setting than this column's other rows, so those two cells are
withdrawn rather than reported; their LIBERO columns are unaffected.
``---'' = not measured.}
