...
\begin{table}[t]
\centering
\caption{LIBERO hard-8 in the \emph{multitask} setting: a single policy and a
single critic trained on all eight tasks jointly, in contrast to the
per-task routing of \cref{tab:libero-single}. Powered evaluation (100
rollouts $\times$ 3 evaluation seeds per cell; one column per training seed;
100 RL iterations). $\Delta$ is measured against each row's \emph{own}
base, which is the only fair comparison across the two blocks: the
flow-matching numbers are larger in absolute terms purely because that base
starts higher. The ``DICE-RL (same base)'' row is the like-for-like
comparison against \method{} --- same drifting base, same protocol. Read
down the $\Delta$ column: \emph{no} fine-tuner, ours included, moves its
base by more than seed noise ($\pm 0.014$).}
\label{tab:libero}
\small
\begin{tabular}{lcccc}
\toprule
...
backprop actor (same base) & 0.657 & 0.655 & 0.668 & $-0.011$ \\
\method{} (top-$k$)        & --- & 0.684 & --- & $+0.013$ \\
\method{} (tilted)         & 0.677 & --- & 0.672 & $+0.004$ \\
\method{} (final recipe)   & 0.655 & --- & 0.659 & $-0.014$ \\
DICE-RL (same base)        & 0.705 & 0.690 & 0.670 & $+0.017$ \\
\midrule
flow-matching base (no RL) & \multicolumn{3}{c}{$0.738 \pm 0.022$} & --- \\
FM + DICE-RL               & 0.756 & 0.735 & 0.737 & $+0.005$ \\
\bottomrule
\end{tabular}
\end{table}
...
\method{} ($0.655$--$0.659$), every resolution variant ($0.66$--$0.70$) and
DICE-RL ($0.670$--$0.705$) all sit within noise of the untuned base
($0.671 \pm 0.014$). DICE-RL's $+0.017$ and \method{}'s best variant at
