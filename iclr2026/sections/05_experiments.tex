...
worse, so the published settings stand. $^{\ddagger}$DPPO is the one row that does \emph{not} share the $0.279$
...
diffusion $+$ DICE-RL \citep{dicerl2026}   & diffusion & 0.279 & 0.861 & $+0.582$ \\
diffusion $+$ DICE-RL $+$ \method{}        & diffusion & 0.279 & \textbf{0.937} & $\mathbf{+0.658}$ \\
\addlinespace
...
result. This table is a head-to-head comparison, \emph{not} an ablation: the
ablations that take \method{} apart are
\cref{tab:constraint,tab:rho-sweep,tab:locus,tab:resolution,tab:libero-ablation}. $^{\dagger}$ these two runs ended before $20$K steps, so their
...
\midrule
frozen base, no RL & \multicolumn{3}{c}{0.877} & \multicolumn{3}{c}{0.382} \\
DICE-RL actor (backpropagate $-Q$) & 0.970$^{\dagger}$ & 0.946$^{\dagger}$ & 0.940 & 0.870 & 0.909 & 0.861 \\
\method{} actor (bounded target displacement) & \textbf{0.988} & \textbf{0.993} & \textbf{0.978} & \textbf{0.912} & \textbf{0.930} & \textbf{0.929} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{\textbf{What post-training adds, and how fast.} 300-episode
  evaluations every 1K steps; line is the seed mean, band the min--max over
  seeds. The dashed horizontal line is the \emph{frozen base} --- everything
  above it is what fine-tuning bought, shaded for \method{}. On square, the
  discriminative task, \method{} adds $54$ points over the base at the
  matched 20K budget and reaches the backprop actor's 20K endpoint about 4K
  steps earlier; the two resolution variants (top-$k$, tilted) stall near
  $0.55$ and are analysed in \cref{sec:exp-resolution}. On can both updates
  approach the ceiling, so the task mainly checks that nothing breaks. Note
  that FM $+$ DICE-RL runs on a \emph{different} (flow-matching) base and so
  is not a same-base comparison; the same-base contrast is \method{} against
  the backprop actor.}
  \label{fig:curves}
\end{figure}
