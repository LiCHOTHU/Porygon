...
\addlinespace
flow matching $+$ DICE-RL \citep{dicerl2026} & flow matching & --- & 0.923--0.934 & --- \\
flow matching $+$ DICE-RL $+$ \method{}      & flow matching & --- & --- & --- \\
\bottomrule
\end{tabular}}
\end{table}
...
$\eta_Q{=}2$ column should not be read across bases. The GRPO column is
sparse because that baseline was run on four of the eight tasks; we leave the
rest blank rather than infer them, and withhold its row mean. Across the five tasks
...
\midrule
Kitchen-1: open drawer, put bowl in   & 0.818 & 0.820 & 0.831 & 0.857 & 0.897 & 0.873 & --- & \textbf{0.900} \\
Kitchen-3: turn on stove, put pan on  & 0.690 & 0.703 & 0.737 & \textbf{0.752} & 0.700 & --- & --- & 0.690 \\
Kitchen-5: ketchup into top drawer    & 0.610 & 0.587 & 0.694 & \textbf{0.729} & 0.690 & 0.682 & 0.700 & \textbf{0.738} \\
Living-2: orange juice into basket    & 0.785 & 0.800 & 0.842 & \textbf{0.853} & 0.917 & 0.915 & 0.897 & 0.897 \\
Living-5: red mug onto plate          & 0.573 & 0.550 & \textbf{0.826} & 0.812 & 0.787 & 0.809 & \textbf{0.883} & 0.878 \\
Study-1: book into front caddy slot   & 0.648 & 0.663 & 0.710 & \textbf{0.752} & 0.683 & 0.690 & --- & 0.667 \\
Study-1: book into right caddy slot   & 0.578 & 0.583 & \textbf{0.657} & 0.648 & 0.620 & 0.592 & 0.603 & 0.608 \\
Study-3: book into front caddy slot   & 0.588 & 0.653 & 0.607 & 0.635 & \textbf{0.733} & 0.708 & --- & 0.670 \\
\midrule
mean over the eight tasks             & 0.661 & 0.670 & 0.738 & \textbf{0.755} & 0.753 & --- & --- & 0.756 \\
\bottomrule
\end{tabular}}
\end{table}
...
(\cref{tab:baselines}).
\textbf{On the red-mug task anchor-only ($0.810$) reads higher than full
\method{} ($0.781$); that gap is not real.} The anchor-only arm's own three
evaluation seeds span $0.77$--$0.85$, so a $0.029$ difference sits inside its
noise. On ketchup, where its seeds span $0.35$--$0.41$, full \method{} leads
by $0.297$ --- far outside it. What survives noise is that full \method{} is
the only configuration above its base on \emph{both} tasks, while anchor-only
lands $0.223$ \emph{below} base on ketchup, i.e.\ worse than not fine-tuning
at all. \textbf{The ketchup task separates the configurations without
overlap}: the matched full-\method{} arm's three evaluation seeds are
$0.68$, $0.58$, $0.62$ (mean $0.627$) against anchor-only's $0.40$, $0.41$,
$0.35$ (mean $0.387$), so \method{}'s \emph{worst} seed beats the ablation's
\emph{best} by $0.17$, with both step-clip rows at exactly $0.000$ beneath
them. That separation does not rely on averaging and is the cleanest
per-task evidence that the pair is required.
$^{\dagger}$\textbf{these two cells are under re-verification.} The three
ablation rows come from the \texttt{field\_st\_B\{NONE,CLIP,ANC\}} arms, and
the only full-\method{} evaluation we can trace to the matching
\texttt{field\_st\_B} arm gives $0.627$ on ketchup rather than the $0.684$
quoted here; we could not reproduce $0.781$/$0.684$ from any single archived
evaluation. The matched-arm evaluations are running and these cells will be
replaced by them. The separation argument above already uses the lower,
verified value, so it does not depend on the resolution.
``---'' = not measured.}
...
$\times$ & $\checkmark$ & base-restoring only   & 0.810 & 0.387 \\
$\checkmark$ & $\checkmark$ & \textbf{full \method{}} & \textbf{0.781}$^{\dagger}$ & \textbf{0.684}$^{\dagger}$ \\
\bottomrule
\end{tabular}
\end{table}
...
see the discussion below. Square was run only for the top block.
``---'' = not measured.}
...
\quad gradient (\method{})       & ---           & \textbf{0.912--0.930} & 0.781$^{\dagger}$ & 0.684$^{\dagger}$ \\
...
\quad $L_2$ penalty $+$ grad clip (DICE-RL) & loss $+$ params & --- & 0.550 & 0.587 \\
...
\quad dead-zone anchor (\method{})          & action target   & --- & 0.781$^{\dagger}$ & 0.684$^{\dagger}$ \\
\bottomrule
\end{tabular}}
\end{table}
