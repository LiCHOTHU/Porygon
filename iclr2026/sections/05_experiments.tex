...
\textbf{Verdict.} The factorial is complete and it separates the two guards
cleanly. Removing the base-restoring term costs everything --- with the step
...
\begin{tabular}{llcc}
\toprule
constraint & where it acts & \makecell{red mug\\onto plate ($0.573$)} & \makecell{ketchup\\into drawer ($0.610$)} \\
\midrule
$L_2$ penalty $+$ grad clip (DICE-RL) & loss $+$ parameters & 0.550 & 0.587 \\
hinge $(\|r\|-\rho)_{+}^{2}$ penalty  & loss                & 0.837 & 0.700 \\
\midrule
always-on pull (\method{}, $\rho{=}0$) & action target      & 0.549 & 0.601 \\
hard projection $\Pi_{\|r\|\le\rho}$   & action target      & \textbf{0.790} & \textbf{0.733} \\
dead-zone anchor (\method{})           & action target      & 0.781 & 0.684 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{lccc}
\toprule
reward field $V_Q$ & square (3 seeds) & \makecell{red mug\\onto plate ($0.573$)} & \makecell{ketchup\\into drawer ($0.610$)} \\
\midrule
...
critic architecture, dense vs.\ sparse reward, mean evaluation return at
the end of training (50 episodes; dense-task base $\approx$430; the sparse
task is scored by its sparse return). Cells give both seeds (42 / 43); all twelve runs
...
\begin{tabular}{lcc}
\toprule
actor update & swingup (dense) & swingup\_sparse \\
\midrule
unconstrained transport      & 508 / 642 & 57 / 514 \\
backprop $-Q$ (DICE-RL form) & 294 / 343 & 4 / 26 \\
\method{}                   & \textbf{802} / \textbf{628} & 23 / 12 \\
\bottomrule
\end{tabular}
\end{table}
