...
Success uses the same 300-episode protocol throughout. The middle block is the
drop-in test: DICE-RL and DICE-RL $+$ \method{} share the same pretrained
...
\midrule
\multicolumn{5}{l}{\emph{same diffusion base, adding \method{} to an existing fine-tuner}} \\
DICE-RL \citep{dicerl2026}          & diffusion & 0.279 & 0.861 & $+0.582$ \\
DICE-RL $+$ \method{}               & diffusion & 0.279 & \textbf{0.937} & $\mathbf{+0.658}$ \\
\midrule
FM + DICE-RL \citep{dicerl2026} & flow matching & --- & 0.923--0.934 & --- \\
drifting $+$ \method{}          & one-step drifting & 0.382 & 0.912--0.930 & $+0.53$--$0.55$ \\
\bottomrule
\end{tabular}
\end{table}
...
last two rows is how the critic's signal reaches the policy}. ``backprop
$-Q$'' differentiates the critic through the policy, the standard choice.
``field-target $+$ constraint'' is \method{}: it displaces sampled actions
along the critic's gradient, bounds that displacement in action space, and
regresses the policy onto the result. $^{\dagger}$ these two runs ended before $20$K steps, so their
...
$\sim\!2$ points elsewhere. ``---'' = unrun cell. The multitask
...
\midrule
Kitchen-1: open drawer, put bowl in   & 0.818 & 0.820 & 0.831 & \textbf{0.857} & 0.897 & 0.873 & --- & --- \\
Kitchen-3: turn on stove, put pan on  & 0.690 & 0.703 & 0.737 & \textbf{0.752} & 0.700 & --- & --- & --- \\
...
Study-1: book into front caddy slot   & 0.648 & 0.663 & 0.710 & \textbf{0.752} & 0.683 & 0.690 & --- & --- \\
Study-1: book into right caddy slot   & 0.578 & 0.583 & \textbf{0.657} & 0.648 & 0.620 & 0.592 & 0.603 & 0.608 \\
Study-3: book into front caddy slot   & 0.588 & 0.653 & 0.607 & \textbf{0.635} & 0.733 & 0.708 & --- & --- \\
\midrule
mean over the eight tasks             & 0.661 & 0.670 & 0.738 & \textbf{0.755} & 0.753 & --- & --- & --- \\
\bottomrule
\end{tabular}}
\end{table}
