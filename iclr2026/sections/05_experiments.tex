...
authors}, whose configurations we run verbatim (their original papers target
dense-reward locomotion or offline D4RL). Their published settings give them
...
$^{\dagger\dagger}$the paired rows are quoted at a matched $496$K
environment steps, the largest budget all six seeds share (mean of last three
evaluations per seed, three seeds each). Both arms saturate at the same
ceiling and are statistically indistinguishable there (\method{}
$0.946$--$0.967$, DICE-RL $0.961$--$0.970$); the difference is the path ---
at a matched $352$K, before saturation, \method{} reads $0.955$ against
DICE-RL's $0.941$, so it reaches the ceiling at less compute.
...
diffusion $+$ DICE-RL $+$ \method{}$^{\P\P}$        & diffusion & 0.600 & 0.959$^{\dagger\dagger}$ & $+0.359$ \\
\addlinespace
...
baseline. $^{\ddagger}$\textbf{transport is the task \method{} loses, and we report it
as such.} At a matched $2.24$M steps (three seeds, converged) the
backpropagated actor reaches $0.912$ where \method{} plateaus at $0.594$. An
earlier snapshot at $256$K read as a tie; the gap opened as training
continued. Transport is the hardest task here (two arms, longest horizon),
and one reading is that its critic, trained on far more successful
experience at this budget, earns the trust that \method{}'s conservatism
withholds. Two tasks favour \method{}, one favours backpropagation ---
the honest summary is that the update rule's advantage is largest where the
critic is weakest, and transport is where it is strongest.}
\label{tab:matched}
\footnotesize
...
DICE-RL actor \emph{(baseline)} & $Q$ differentiated \emph{through} the policy & 0.970$^{\dagger}$ & 0.946$^{\dagger}$ & 0.940 & 0.870 & 0.909 & 0.861 & \textbf{0.908} & \textbf{0.937} & \textbf{0.891} \\
\textbf{\method{} actor \emph{(ours)}} & $Q$ only \emph{queried}; targets displaced, bounded, regressed & \textbf{0.988} & \textbf{0.993} & \textbf{0.978} & \textbf{0.912} & \textbf{0.930} & \textbf{0.929} & 0.569 & 0.634 & 0.578 \\
\bottomrule
\end{tabular}}
\end{table}
...
a loss-space hinge that beats our anchor on two of three tasks.
...
necessary. Three seeds per cell.}
...
data, it is what removes any route back. The lower block stages the
expansion: per task we report the demonstration average (what the distilled
base should score), the best single demonstration (the bar an arm must clear
to beat its data), and the SAC teacher's final return --- all measured, with
the three arms training. Humanoid \emph{run} and manipulator bring-ball are
excluded: their teachers plateaued at returns $10$ and $0.2$ respectively,
below demonstration quality (the manipulator teacher is retraining at a
$4$M-step budget).}
\label{tab:dmc-scope}
\small
\resizebox{\linewidth}{!}{%
...
\midrule
\multicolumn{5}{l}{\emph{new tasks, same three arms (training). Reference points measured from our data:}} \\
\emph{task} & demo avg ($=$ base) & best demo (the bar) & SAC teacher & arms \\
quadruped run              & 829 & 952  & 914 & training \\
walker run                 & 522 & 706  & 691 & training \\
humanoid walk              & 451 & 575  & 468 & training \\
cartpole balance           & 931 & 1000 & 995 & training \\
cartpole balance (sparse)  & 807 & 1000 & same teacher & training \\
\bottomrule
\end{tabular}}
\end{table}
