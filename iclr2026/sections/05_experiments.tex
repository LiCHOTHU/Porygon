...
\begin{table}[t]
\centering
\caption{\textbf{LIBERO hard-8, per-task fine-tuning} (task-ID routing at
deployment, the architecture published LIBERO fine-tuners use). Powered
evaluation: $100$ rollouts $\times$ $3$ evaluation seeds; \method{} averages
its last three checkpoints, with two training seeds on the ketchup,
orange-juice and red-mug tasks (seed spread $0.000$--$0.089$; treat
same-size gaps as noise). \textbf{One shared recipe across all eight tasks}
--- per-task tuning would raise our numbers (\cref{tab:eta-sweep}) but give
us a budget the baselines lack. \emph{Reading rules:} each block's columns
share that block's base, so comparisons are within-block only; bold marks
the best fine-tuning method per row (never the no-RL reference, never across
blocks). $\method{}_{\eta 1}$/$\method{}_{\eta 2}$ differ only in the reward
step; the cross-base comparison is $\eta_Q{=}1$ against $\eta_Q{=}1$ (means
$0.738$ and $0.756$), as the flow-matching $\eta_Q{=}2$ run does not exist.
$^{\ddagger}$the four marked GRPO cells are provisional --- shorter runs
under a single $20$-rollout evaluation --- so the GRPO mean mixes protocols
($0.771$ over its four matched cells). ``---'' = unrun; row means with gaps
are withheld. The multitask version of this comparison is
\cref{tab:libero}.}
...
data, it is what removes any route back. The lower block reports the expansion in progress, one seed per arm.
``base'' is the \emph{measured} return of the distilled base, with the
demonstration average it was distilled from in parentheses --- on
cartpole-balance the distill preserves far less of the teacher than on
swingup, so gains must be read against the measured base.
$^{\circ}$still training, quoted at the current budget;
$^{\mathrm{v}}$high evaluation variance ($490$--$908$ across its last five).
The one converged cell is the sparse \method{} arm: at the $1000$ ceiling ---
the maximum return the environment can emit --- for five consecutive
evaluations spanning $100$K steps, from a base of $90$.
$^{\mathrm{d}}$the walker and humanoid bases lost nearly all teacher
performance in distillation ($17$ from a $691$ teacher; $1$ from $468$);
those rows currently measure the distill, not the fine-tuner, and a longer
distill is running. Unconstrained-arm cells fill as those runs mature.
Humanoid \emph{run} and manipulator bring-ball are excluded: their teachers
plateaued at $10$ and $0.2$ (the manipulator teacher is retraining at
$4$M steps).}
\label{tab:dmc-scope}
\small
\resizebox{\linewidth}{!}{%
...
\midrule
\multicolumn{5}{l}{\emph{new tasks, single seed, quoted at the current budget ($200$--$260$K steps)$^{\circ}$:}} \\
\emph{task} & base (demo avg) & backprop $-Q$ & \method{} & unconstrained \\
cartpole balance           & 378 (931) & \textbf{834}$^{\circ}$ & 823$^{\circ}$ & --- \\
cartpole balance (sparse)  & 90 (807)  & 795$^{\circ\mathrm{v}}$ & \textbf{1000} & --- \\
quadruped run              & 238 (829) & 267$^{\circ}$ & 246$^{\circ}$ & --- \\
walker run                 & 17 (522)$^{\mathrm{d}}$  & 29$^{\circ}$ & 23$^{\circ}$ & --- \\
humanoid walk              & 1 (451)$^{\mathrm{d}}$   & 1$^{\circ}$  & 1$^{\circ}$  & --- \\
\bottomrule
\end{tabular}}
\end{table}
