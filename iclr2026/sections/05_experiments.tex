...
\begin{table}[t]
\centering
\caption{Dead-zone radius sweep on LIBERO ($\rho{=}0$ is an always-on
penalty; $\rho{=}\infty$ disables the restoring term). The interior of the
range is flat; the endpoints fail differently (see text).}
\label{tab:rho-sweep}
\small
\begin{tabular}{lcc}
\toprule
...
distilled base}, dense vs.\ sparse reward. Scores are episode return
($0$--$1000$); swingup rows average two seeds, new tasks one.
$^{\circ}$still training; $^{\mathrm{v}}$high eval variance; $^{\mathrm{t}}$tuned reward step (see text).}
\label{tab:dmc-scope}
\small
\begin{tabular}{lcccc}
\toprule
...
cartpole balance      & 378 & 815$^{\circ}$ & 844$^{\circ}$ & \textbf{911}$^{\circ}$ \\
quadruped run         & 238 & 97$^{\circ}$ & 283$^{\circ}$ & \textbf{312}$^{\circ\mathrm{t}}$ \\
\midrule
\multicolumn{5}{l}{\emph{sparse reward}} \\
cartpole balance (sparse) & 90  & 880$^{\circ}$ & 843$^{\circ\mathrm{v}}$ & \textbf{1000} \\
\bottomrule
\end{tabular}
\end{table}
...
$23$ points and the critical one is task-dependent (see text).}
