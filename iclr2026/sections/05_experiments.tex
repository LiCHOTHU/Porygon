...
the gap.
$^{\ddagger}$\textbf{DPPO now trains from the same checkpoint as every other
diffusion row} (\texttt{square\_diff\_baselinearch\_42}); its earlier
$0.563 \rightarrow 0.876$ used a different pretrained policy and is
superseded. The retrain is still in progress --- $0.561$ at $368$K of the
$\sim\!12$M environment steps its schedule needs --- so its fine-tuned cell
is a lower bound and will rise.
One measurement complicates the base column and we state it rather than hide
it. The diffusion rows all load one file, but they do not all score it the
same: the classic baselines' harness reads it as $0.279$ and DPPO's reads the
\emph{same file} as $0.520$. We traced this to the samplers rather than the
weights --- each method wraps the pretrained denoiser in its own sampling
loop --- but removing the obvious candidate (the $0.1$ noise floor those
loops impose) moved the number by $0.001$, so the mechanism is not yet
identified. Each row therefore reports its base as its own harness measures
it, and $\Delta$ is always within-harness; cross-row base comparison is not
meaningful.
The flow-matching base measures $0.570$ on square (300 episodes).
``---'' = not measured.}
\label{tab:baselines}
\footnotesize
...
DPPO \citep{ren2024dppo}$^{\ddagger}$ & diffusion & 0.520 & 0.561$^{\ddagger}$ & $+0.041$ \\
\midrule
...
and red-mug tasks. \textbf{One recipe across all eight tasks, no per-task
tuning}: every \method{} cell uses a single shared reward step, chosen for its
mean over the eight tasks rather than per task. The full sweep is
\cref{tab:eta-sweep} --- per-task optima disagree ($\eta_Q{=}4$ is best on the
red-mug task, $\eta_Q{=}1$ on ketchup), and reporting each task at its own
optimum would raise our numbers while giving us a tuning budget the baselines
do not get.
...
\quad top-$k$ scoring (mean $Q$ of best $k$) & 0.679 & 0.684 & 0.669 & $+0.006$ \\
