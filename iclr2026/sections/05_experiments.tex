...
difference within a pair isolates the update rule. The flow-matching pair is
incomplete --- its \method{} cell is still training at matched budget --- and
we leave it blank rather than pair it against a run of different length.
...
therefore a lower bound on what the full schedule would reach. ``---'' = not
measured; we have not run the flow-matching base on square, so its $\Delta$
is unavailable.}
\label{tab:baselines}
\footnotesize
...
\midrule
DIPO $+$ displacement bound & diffusion & 0.279 & 0.198 & $-0.081$ \\
\midrule
...
placed on the displacement itself is what these methods lack, which is why
\cref{tab:baselines}'s DIPO $+$ displacement bound row exists.
