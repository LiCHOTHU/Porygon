...
worse, so the published settings stand. $^{\P\P}$\textbf{Every diffusion row shares one base.} All diffusion rows
(the five classic baselines, DPPO, and the paired DICE-RL rows) start from a
single retrained checkpoint that measures $\mathbf{0.600}$ on $300$ episodes
($24000$ epochs, $[1024,1024,1024]$ trunk with a conditioning MLP, $20$
denoising steps), independently confirmed at $0.550$--$0.600$ by the step-$0$
evaluations of all six paired fine-tuning seeds. This replaces the earlier
$0.279$ base, whose weakness we could not defend; results obtained on it are
superseded and no longer shown. ``training'' cells fill as the re-runs reach
their matched budget ($3$ seeds for the paired rows, seed $42$ for the
baselines and DPPO). The flow-matching base measures $0.570$ on square
($300$ episodes). QSM and DQL collapse to $0.000$ on this base exactly as they
did on the weak one ($14{+}$ consecutive zero evaluations each), which rules
out base quality as the explanation for their failure; a collapse to zero
reads the same under every success rule, so the metric difference between
harnesses cannot account for it either.
$^{\dagger\dagger}$the paired rows are quoted at a matched $352$K
environment steps, the largest budget all six seeds share (mean of last three
evaluations per seed): \method{} $0.959/0.953/0.952$ against DICE-RL's
$0.929/0.953/0.942$. Both continue to improve slightly past this point
(DICE-RL reaches $0.966$ by $720$K); the matched comparison is the one we
argue from.
$^{*}$every $\Delta$ is exactly fine-tuned minus the printed base. One
caution when reading the starred rows: their harness scores success more
strictly (a full action chunk inside the success state) and reads this same
checkpoint at $0.348$ under that rule, so up to $0.252$ of each starred drop
is the measurement rule rather than degradation --- under its own rule, e.g.,
AWR gains $+0.045$ over its $0.348$ reading. The collapse rows ($0.000$) are
unaffected by this caveat. Drifting and flow-matching rows are means over
three seeds; per-seed values for the drifting pair are in \cref{tab:matched}. DPPO's and the
paired rows' harnesses read the base at $0.59$--$0.60$, matching the base
column. $^{\circ}$still training; value at the current budget.
``---'' = not measured.}
\label{tab:baselines}
\footnotesize
...
DPPO \citep{ren2024dppo} & diffusion & 0.600 & 0.632$^{\circ}$ & $+0.032$ \\
\midrule
\multicolumn{5}{l}{\emph{adding \method{} to DICE-RL, paired within each base}} \\
diffusion $+$ DICE-RL \citep{dicerl2026}$^{\P\P}$   & diffusion & 0.600 & 0.941$^{\dagger\dagger}$ & $+0.341$ \\
diffusion $+$ DICE-RL $+$ \method{}$^{\P\P}$        & diffusion & 0.600 & \textbf{0.955}$^{\dagger\dagger}$ & $\mathbf{+0.355}$ \\
\addlinespace
...
\begin{table}[t]
\centering
\caption{\textbf{Dense-reward suite (gym locomotion): the \cref{tab:baselines}
roster under dense reward.} Hopper, Walker2d and HalfCheetah (medium D4RL
data), the environments the diffusion-RL baselines' own papers report on; each
method runs its official released configuration, and DICE-RL/\method{} share
the same pretrained diffusion base per task. Scores are normalized episode
return. Every method fine-tunes from the same epoch-$1000$ pretrained base
per task (the official pretrain schedule runs to $3000$; BC quality saturates
far earlier, and sharing one checkpoint keeps the eight-way comparison
internally fair). All runs are training at the time of writing; cells fill as
each converges, and no cell will be quoted before its curve supports it.}
\label{tab:gym-dense}
\small
\begin{tabular}{lccc}
\toprule
...
\midrule
DIPO   & training & training & training \\
QSM    & training & training & training \\
DQL    & training & training & training \\
IDQL   & training & training & training \\
AWR    & training & training & training \\
DPPO   & training & training & training \\
\midrule
diffusion $+$ DICE-RL              & training & training & training \\
diffusion $+$ DICE-RL $+$ \method{} & training & training & training \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\multicolumn{5}{p{0.95\linewidth}}{\emph{in progress, same three arms:
dense/sparse pairs acrobot swingup (teacher retraining at a $10\times$ budget)
and cartpole balance; dense-only harder robots walker run, quadruped run,
humanoid walk, humanoid run, and manipulator bring-ball. Cells fill as each
task's teacher, distilled base and arms complete.}} \\
\bottomrule
\end{tabular}}
\end{table}
