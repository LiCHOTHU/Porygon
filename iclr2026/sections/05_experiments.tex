...
baseline. $^{\ddagger}$\textbf{transport is a tie and is still training.} Unlike the can and square blocks, which are converged, these are a snapshot at a matched $256$K steps --- the largest step at which all six runs have an evaluation --- and both arms are still climbing (one \method{} seed reaches $0.46$ by $480$K). The three-seed means are $0.323$ (DICE-RL) against $0.331$ (\method{}) with fully overlapping seed ranges --- a tie, reported because a task that does \emph{not} separate the update rules belongs in the table too.}
\label{tab:matched}
\footnotesize
