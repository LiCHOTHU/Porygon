...
Modern generative policies sharpen the problem, because they remove the
usual escape route. A policy that maps noise to a multimodal action through
a learned sampler has no tractable likelihood, so the ratio-based toolbox
--- PPO, and anything else built on importance weights --- is unavailable.
This is true of diffusion and flow-matching policies and most acute for
one-step samplers such as a drifting model \citep{deng2026drifting}, where a
single network evaluation stands between noise and action. It can be recovered
...
actions (\cref{sec:exp-robustness}), and that is fine. The claim is that the
...
The construction assumes very little about what it is attached to. It needs
a frozen map from noise to actions, $K$ samples from it, and a critic; it
never inspects the generator's internals, never needs a likelihood, and
never assumes a denoising objective. Consequently it applies unchanged to
one-step, few-step, and many-step samplers, and we verify it on two of them.
One economy is worth a remark but no more: a drifting model is itself
pretrained by field-target regression \citep{deng2026drifting}, so on that
base the fine-tuner happens to reuse machinery the policy already has. That
is convenience, not requirement, and no claim in this paper rests on it.
...
\begin{itemize}
  ...
  (\cref{sec:exp-constraint}). Two further choices separate our instantiation
