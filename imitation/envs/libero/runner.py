...
from imitation.envs.utils import FrameStackObservationFixed
import wandb
...
class LiberoRunner():
    ...
    def run_episode(self, env, env_name, policy, init_states_, env_num, render=False):
        ...
        # plt.imshow(obs['agentview_rgb'][0,0, ::-1])
        # plt.show()
        # return 0, 0, {}
        ...
        if self.test_inference_time:
            ...
            print('Hz:', hz)
            exit()
        for steps in trange(self.max_episode_length, disable=False):
        # while steps < self.max_episode_length and not done:
            # import torch
            # x = [param.size() for param in policy_object.parameters()]
