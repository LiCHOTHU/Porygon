...
class DexMimicGenRunner():
    ...
    def get_init_states(self, env_name):
        if self.deterministic:
            ...
            if self.robot == 'Panda':
                fpath = os.path.join(os.path.dirname(fpath), 'utils', 'mg_init', f'{env_name}.init')
            else:
    ...
    def run(
        ...
        env_metrics = []
        env_fn = lambda: DexMimicGenFrameStack(self.env_factory(env_name=env_name), self.frame_stack)
        # init_states = self.get_init_states(env_name=env_name)
        init_states = None # TODO
        start = len(progress['successes'])
    ...
    def run_policy_in_env(self, policy, env_fn, render=False, init_states=None, start=0, do_tqdm=False):
        for i in range(start, self.rollouts_per_env):
            env = env_fn()
            # print("Rendering: ", render)
            success, total_reward, episode = self.run_episode(env, policy, render, do_tqdm=do_tqdm)
