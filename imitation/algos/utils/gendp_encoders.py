...
class PointNetSetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        new_points = einops.rearrange(new_points, 'b npoint nsample cd -> b cd nsample npoint')
        ...
        new_points = einops.rearrange(new_points, 'b d n -> b n d')
...
class PointNet2Encoder(nn.Module):
    def __init__(self, in_shape, out_channels=256, use_bn=False):
        ...
        # pn2.1_r_0.04
        self.sa1 = PointNetSetAbstraction(npoint=512, radius=0.04, nsample=32, in_channel=in_channels + 3, mlp=[64, 64, 128], group_all=False)
    ...
    def forward(self, xyz, points, mask=None, **kwargs):
        B, Fs, N, C = xyz.size()
        xyz = einops.rearrange(xyz, 'b fs n c -> (b fs) n c')
        points = einops.rearrange(points, 'b fs n d -> (b fs) n d')
        ...
        x = einops.rearrange(x, '(b fs) d -> b fs d', b=B)
        return x
...
class PointNetAttnEncoder(nn.Module):
    def __init__(self, in_shape, out_channels, use_bn=False):
