...
from imitation.algos.utils.custom_attn_processor import SeparateKVAttnProcessor
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        self.attn1 = Attention(
            ...
            processor=SeparateKVAttnProcessor()
        )
    ...
    def forward(
        ...
        attn_output = self.attn1(
            ...
            value_encoder_hidden_states=value_encoder_hidden_states,
...
class DiT(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        interleave_self_and_separate_attention=False,
    ):
        super().__init__()

        assert not (interleave_self_attention and interleave_self_and_separate_attention), \
            'Can only choose one interleave option'
        self.attention_head_dim = attention_head_dim
    ...
    def forward(
        ...
        value_encoder_hidden_states: torch.Tensor = None,
        ...
        for idx, block in enumerate(self.transformer_blocks):
            if self.config.interleave_self_attention:
                value_encoder_hidden_states_input = None
                if idx % 2 == 0:
                    encoder_hidden_states_input = encoder_hidden_states
                else:
                    encoder_hidden_states_input = None
            elif self.config.interleave_self_and_separate_attention:
                if idx % 3 == 0:
                    encoder_hidden_states_input = encoder_hidden_states
                    value_encoder_hidden_states_input = None
                elif idx % 3 == 1:
                    encoder_hidden_states_input = encoder_hidden_states
                    value_encoder_hidden_states_input = value_encoder_hidden_states
                else:
                    encoder_hidden_states_input = None
                    value_encoder_hidden_states_input = None
            else:
            ...
            hidden_states = block(
                    ...
                    value_encoder_hidden_states=value_encoder_hidden_states_input,
                    encoder_attention_mask=encoder_attention_mask,
                    temb=temb,
                )
