...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if traj_transform is not None:
            ...
            self.traj_transform = lambda x: x
    ...
    def normalize_action(self):
        # TODO: 1st and 99th percentile computation
        """
        Computes a dataset-wide mean and standard deviation for the observations 
        (per dimension and per obs key) and returns it.
        """
        def _compute_traj_stats(traj_obs_dict):
            ...
            for k in traj_obs_dict:
                traj_stats[k]["n"] = traj_obs_dict[k].shape[0]
                traj_stats[k]["mean"] = traj_obs_dict[k].mean(axis=0) # [1, ...]
                traj_stats[k]["sqdiff"] = ((traj_obs_dict[k] - traj_stats[k]["mean"]) ** 2).sum(axis=0) # [1, ...]
                traj_stats[k]['max'] = traj_obs_dict[k].max(axis=0)
                traj_stats[k]['min'] = traj_obs_dict[k].min(axis=0)
            return traj_stats
        ...
        # with the previous statistics.
        ep = self.demos[0]
        action_traj = {k: self.hdf5_file["data/{}/{}".format(ep, k)][()].astype('float32') for k in self.dataset_keys}
        action_traj = self.traj_transform(action_traj)
        ...
        for ep in self.demos[1:]:
            action_traj = {k: self.hdf5_file["data/{}/{}".format(ep, k)][()].astype('float32') for k in self.dataset_keys}
            all_actions.extend(action_traj['actions'])
            # all_abs_actions.extend(action_traj['abs_actions'])
            action_traj = ObsUtils.process_obs_dict(action_traj)
