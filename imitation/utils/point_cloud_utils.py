...
def lift_point_cloud_batch(depths, intrinsics, extrinsics, keepdims=False) -> torch.Tensor:
    ...
    # intrinsics = torch.split(intrinsics, 1, dim=-1)  # fx, fy, cx, cy
...
# def pos_6d_to_matrix(pos_euler):
...
def show_point_cloud_viser(pcd, pcd_colors=None, vectors=None, extra_points=None, port=8080):
    ...
    # for axis, (end, color) in axes.items():
    #     server.add_line(
    #         name=f"axis_{axis}",
    #         start=origin,
    #         end=end,
    ...
    # # Add direction vectors if provided
    # if vectors is not None:
    #     if isinstance(vectors, torch.Tensor):
    #         vectors = vectors.detach().cpu().numpy()
        
    #     for i, (start, direction) in enumerate(zip(pcd, vectors)):
    #         server.add_line(
    #             name=f"vector_{i}",
    #             start=start,
